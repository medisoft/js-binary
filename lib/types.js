...
module.exports.hex = {
    write: function (s, data, path) {
        if (typeof s !== 'string') {
            throw new TypeError('Expected a hex string at ' + path + ', got ' + s)
        }

        exports.Buffer.write(new Buffer(s,'hex'), data, path)
    },
    read: function (state) {
        return exports.Buffer.read(state).toString('hex')
    }
}
...
module.exports.vint = {
    write: function (s, data, path) {
        // Check the input
        if (Math.round(s) !== s || s > MAX_DOUBLE_INT || s < 0) {
            throw new TypeError('Expected unsigned integer at ' + path + ', got ' + s)
        }

        exports.Buffer.write(new Buffer(varint.encode(s)), data, path)
    },
    read: function (state) {
        return varint.decode(exports.Buffer.read(state))
    }
}
